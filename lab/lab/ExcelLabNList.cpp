...
{
	ext					= "lbn";
	xl_filter_name		= "Список лабораторных номеров";
	xl_form				= "LabNLabList";
}
	...
	try
	{
		...
		for (int r = fr; ;r++)
		{
			...
			else
			{
				char str[1028];
				sprintf(str, 
					"Обработано %d строк с лабораторными номерами\n"
					"Загружено %d лабораторных номеров\n"
					"Из их перезаписано %d лабораторных номеров"
					, r-fr
					, r-fr-perezapisano
					, perezapisano);
				AfxMessageBox(str);
			...
			else
			{
				char str[1028];
				sprintf(str, 
					"Отсутствует глубина в строке %d для лабораторного номера %d"
					, r, LAB_N);
				AfxMessageBox(str);
			}
			...
			else
			{
				char str[1028];
				sprintf(str, 
					"Отсутствует ID_KT в строке %d для лабораторного номера %d"
					, r, LAB_N);
				AfxMessageBox(str);
			}
			...
			{
				char str[1028];
				sprintf(str, "Лабораторный номер (строка=%d) LAB_N=%d ID_PROB=%d \nуже имеется в базе \nон будет отредактирован"
					,r , LAB_N, ID_PROB);
				AfxMessageBox(str);
