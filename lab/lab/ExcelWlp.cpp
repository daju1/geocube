// ExcelWLP.cpp: implementation of the ExcelWLP class.
//
//////////////////////////////////////////////////////////////////////
...
void ExcelWLP::InitExt()
{
	...
	case 2:
		{
			ext					= "wlp";
			xl_filter_name		= "W текучести и раскатывания";
			xl_form				= "WLP";
		}
		break;
	case 3:
	...
	case 2:
		{
			...
			for (int r = 0; r <= 11; r++)
			{
				...
				sprintf(S, "M%d", row1);
				sprintf(F, "=(J%d-K%d)/(K%d-L%d)", row1, row1, row1, row1);
				VERIFY(pRange = m_excel.m_worksheet.GetRange(COleVariant(S)));
				m_range.AttachDispatch(pRange);		
				m_range.SetValue(COleVariant(F));
				m_range.SetNumberFormat(COleVariant("0.000"));
				...
				{
					sprintf(S, "M%d", row2);
					sprintf(F, "=(J%d-K%d)/(K%d-L%d)", row2, row2, row2, row2);
					VERIFY(pRange = m_excel.m_worksheet.GetRange(COleVariant(S)));
					m_range.AttachDispatch(pRange);		
					m_range.SetValue(COleVariant(F));
					m_range.SetNumberFormat(COleVariant("0.000"));
	...
	case 3:
		{
			...
			for (int r = 0; r <= 7; r++)
			{
				...
				{
					sprintf(S, "M%d", row3);
					sprintf(F, "=(J%d-K%d)/(K%d-L%d)", row3, row3, row3, row3);
					VERIFY(pRange = m_excel.m_worksheet.GetRange(COleVariant(S)));
					m_range.AttachDispatch(pRange);		
					m_range.SetValue(COleVariant(F));
					m_range.SetNumberFormat(COleVariant("0.000"));
