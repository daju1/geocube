...
/*
 * gstat.c: program flow, init/gstat_exit procedures
 */
#include <stdio.h>
#include <stdlib.h> 
...
-c    check command file syntax and gstat_exit\n\
...
static void parse_options(int argc, char *argv[]) {
	...
	if (argc > 1 && almost_equals(argv[1], "-e$xecute"))
		gstat_exit(exec_action(argc - 2, argv + 2));
