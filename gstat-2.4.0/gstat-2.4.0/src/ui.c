...
/* use pipe? */
	...
	} /* gstat_exit event loop while (! quit) */
#ifdef HAVE_POPEN
	if (gnu_stream != NULL)
		epclose(gnu_stream);
#endif
...
#endif
	return (key == 'x' || key == 'X'); /* we quit by pressing gstat_exit */
}
...
void show_plot(int key) {
...
#endif
	{
		...
		if (esystem(cp)) /* do it */
			msg = "Error (non-zero gstat_exit) from gnuplot";
		efree(cp);
...
#endif
			if (esystem(cmd))
				msg = "Error (non-zero gstat_exit) from jgraph";
			else
