...
ApiHook* ApiHook::sm_pHead = NULL;

// By default, the module containing the ApiHook() is not hooked
BOOL ApiHook::ExcludeAPIHookMod = FALSE;
...
PROC ApiHook::sm_ProcLoadLibraryA   = 0;
PROC ApiHook::sm_ProcLoadLibraryW   = 0;
PROC ApiHook::sm_ProcLoadLibraryExA = 0;
PROC ApiHook::sm_ProcLoadLibraryExW = 0;
PROC ApiHook::sm_ProcGetProcAddress = 0;


ApiHook ApiHook::sm_LoadLibraryA  ("Kernel32.dll", "LoadLibraryA",     (PROC) ApiHook::LoadLibraryA,   &sm_ProcLoadLibraryA);

ApiHook ApiHook::sm_LoadLibraryW  ("Kernel32.dll", "LoadLibraryW",     (PROC) ApiHook::LoadLibraryW,   &sm_ProcLoadLibraryW);

ApiHook ApiHook::sm_LoadLibraryExA("Kernel32.dll", "LoadLibraryExA",   (PROC) ApiHook::LoadLibraryExA, &sm_ProcLoadLibraryExA);

ApiHook ApiHook::sm_LoadLibraryExW("Kernel32.dll", "LoadLibraryExW",   (PROC) ApiHook::LoadLibraryExW, &sm_ProcLoadLibraryExW);

ApiHook ApiHook::sm_GetProcAddress("Kernel32.dll", "GetProcAddress",   (PROC) ApiHook::GetProcAddress, &sm_ProcGetProcAddress);



ApiHook::ApiHook(PSTR  pszCalleeModName,
                 PSTR  pszFuncName,
                 PROC  pfnHook,
                 PROC* pfnOrig)
{
...
FARPROC ApiHook::GetProcAddressRaw(HMODULE hmod,
                                   PCSTR   pszProcName)
{
   ProcGetProcAddress proc = (ProcGetProcAddress)sm_ProcGetProcAddress;

   if (proc)
      return(proc(hmod, pszProcName));
...
void ApiHook::ReplaceIATEntryInAllMods(PCSTR pszCalleeModName,
                                       PROC  pfnCurrent,
                                       PROC  pfnNew)
{
...
void ApiHook::ReplaceIATEntryInOneMod(PCSTR   pszCalleeModName, 
                                      PROC    pfnCurrent,
                                      PROC    pfnNew,
                                      HMODULE hmodCaller)
{
...
void ApiHook::ReplaceEATEntryInOneMod(HMODULE hmod,
...
void ApiHook::FixupNewlyLoadedModule(HMODULE hmod,
                                     DWORD   dwFlags)
{
   ...
   {
      for (ApiHook* p = sm_pHead; p != NULL; p = p->m_pNext)
      {
         if (p->m_pfnOrig != NULL)
         {
...
{
   ProcLoadLibraryA proc = (ProcLoadLibraryA)sm_ProcLoadLibraryA;

   HMODULE hmod = proc(pszModulePath);
...
{
   ProcLoadLibraryW proc = (ProcLoadLibraryW)sm_ProcLoadLibraryW;
...
HMODULE WINAPI ApiHook::LoadLibraryExA(PCSTR  pszModulePath,
                                        ...
                                        DWORD  dwFlags)
{
   ProcLoadLibraryExA proc = (ProcLoadLibraryExA)sm_ProcLoadLibraryExA;

   HMODULE hmod = proc(pszModulePath, hFile, dwFlags);
...
HMODULE WINAPI ApiHook::LoadLibraryExW(PCWSTR pszModulePath,
                                       ...
                                       DWORD  dwFlags)
{
   ProcLoadLibraryExW proc = (ProcLoadLibraryExW)sm_ProcLoadLibraryExW;
...
FARPROC WINAPI ApiHook::GetProcAddress(HMODULE hmod,
                                       PCSTR   pszProcName)
{
