...
public:
  ...
  //мои добавки - оператор присваивания и конструктор копирования
  void Init(const dimeArray& ob);
  dimeArray& operator=(const dimeArray& ob);
  dimeArray(const dimeArray& ob);
...
template <class T> inline dimeArray<T>& 
dimeArray<T>::operator=(const dimeArray<T>& ob)
{
	if (this == &ob)
		return *this;
	this->Init(ob);
...
template <class T> inline void 
dimeArray<T>::Init(const dimeArray<T>& ob)
{
	if (!this->array || this->size != ob.size)
	{
	...
	this->num		= ob.num;
	for (int i = 0; i < this->size; i++) this->array[i] = ob.array[i];
}

template <class T> inline 
dimeArray<T>::dimeArray(const dimeArray<T>& ob)
{
	this->Init(ob);
}
template <class T> inline void 
dimeArray<T>::resize(const int sz)
{
	if (!this->array || this->size != sz)
	{
		if (this->array) delete [] this->array;
		this->size		= sz;
		this->array		= new T[this->size];
