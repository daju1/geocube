...
void dimeBSPTree::Init(const dimeBSPTree& ob){
	//dimeArray <dimeVec3f> pointsArray;
	this->pointsArray = ob.pointsArray;
	//dimeArray <void*> userdataArray;
	this->userdataArray = ob.userdataArray;
	//dime_bspnode *topnode;
	this->topnode = new dime_bspnode(&this->pointsArray);//??????????????????????Left???Right???
	//int maxnodepoints;
	this->maxnodepoints = ob.maxnodepoints;
	//dimeBox *boundingBox;
	//вызываем конструктор копирования
	this->boundingBox = new dimeBox(*ob.boundingBox);
}
