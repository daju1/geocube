...
dimeInput::skipWhiteSpace()
{
  ...
  // while((gotChar = get(c)) && isspace(c) && c != endline && c != endline2);//original fail on russian chars
  while((gotChar = get(c)) && ((c >= 0x09 && c <= 0x0D) || c == 0x20) && c != endline && c != endline2);//corrected varianf for do not fail on russian chars
  /*while(true)
  {
	  bool r1 = (gotChar = get(c));
	  //bool r2 = isspace(c);// fails here !!!
	  bool r2 = (c >= 0x09 && c <= 0x0D) || c == 0x20;
	  bool r3 = c != endline;
	  bool r4 = c != endline2;
	  bool rez = r1 && r2 && r3 && r4;
	  if (!rez)
		  break;
  }*/
