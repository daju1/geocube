...
dxfLineSegment::convert(dxfLineSegment *prev, dxfLineSegment *next,
			dxfLayerData *layerData, dimeMatrix *matrix, const char * layername)
{  
//printf("dxfLineSegment::convert\n");
  this->calculate_v();
  ...
  if (this->w[0] > 0.0 || this->w[1] > 0.0) {
    ...
    layerData->addQuad(v0, v1, v3, v2, layername, matrix);
    
    if (thickness != 0.0) {   
      layerData->addQuad(v0+e, v1+e, v3+e, v2+e, layername, matrix);
      layerData->addQuad(v0, v1, v1+e, v0+e, layername, matrix); 
      if (!next) layerData->addQuad(v1, v3, v3+e, v1+e, layername, matrix);
      layerData->addQuad(v3, v2, v2+e, v3+e, layername, matrix);
      if (!prev) layerData->addQuad(v2, v0, v0+e, v2+e, layername, matrix);
    }
  }
  else {
    if (thickness != 0.0) {
      layerData->addQuad(p[0], p[1], p[1]+e, p[0]+e, layername, matrix); 
    }
    else {
      layerData->addLine(p[0], p[1], layername, matrix);
    }
  }
}
...
void 
dxfLineSegment::convert(dxfLineSegment *prev, dxfLineSegment *next,
			BlnProfile3D *profile, dimeMatrix *matrix, const char * layername, const int colidx)
{  
//printf("dxfLineSegment::convert\n");
  this->calculate_v();
  ...
  if (this->w[0] > 0.0 || this->w[1] > 0.0) {
    ...
    profile->addQuad(v0, v1, v3, v2, layername, colidx, matrix);
    
    if (thickness != 0.0) {   
      profile->addQuad(v0+e, v1+e, v3+e, v2+e, layername, colidx, matrix);
      profile->addQuad(v0, v1, v1+e, v0+e, layername, colidx, matrix); 
      if (!next) profile->addQuad(v1, v3, v3+e, v1+e, layername, colidx, matrix);
      profile->addQuad(v3, v2, v2+e, v3+e, layername, colidx, matrix);
      if (!prev) profile->addQuad(v2, v0, v0+e, v2+e, layername, colidx, matrix);
    }
  }
  else {
    if (thickness != 0.0) {
      profile->addQuad(p[0], p[1], p[1]+e, p[0]+e, layername, colidx, matrix); 
    }
    else {
      profile->addLine(p[0], p[1], layername, colidx, matrix);
    }
  }
}
