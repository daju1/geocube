	    ...
	    dxfLayerData *layerData, dxfConverter *converter)
{
  ...
  for (i = 1; i < numpts; i++) {
    ...
    if (thickness == 0.0) {
      ...
      layerData->addQuad(prev, v, v + e * thickness, prev + e * thickness, entity->getLayerName(),
			 &matrix);
    }
    prev = v;
    rad += inc;
...
void 
profile_arc(const dimeEntity *entity, const dimeState *state, 
	    BlnProfile3D *profile, dxfConverter *converter)
{
  ...
  for (i = 1; i < numpts; i++) {
    ...
    if (thickness == 0.0) {
      ...
      profile->addQuad(prev, v, v + e * thickness, prev + e * thickness, entity->getLayerName(), entity->getColorNumber(),
			 &matrix);
    }
    prev = v;
    rad += inc;
...
void 
yscale_arc(const dimeEntity *entity, const dimeState *state, 
...
void 
xyscale_arc(const dimeEntity *entity, const dimeState *state, 
	    dxfLayerData *, dxfConverter *converter)
{
  dimeArc *arc = (dimeArc*) entity;
