...
ulong CLayerKohonen::GetMinEuclideanDistanceIndex()
{
	ulong	i, m = 0;
	float	f, min = FLT_MAX;
	...
	for (i = 0; i < m_rang; i++)
	{
		f = ((CNeuronKohonen*) m_neurons[i])->GetEuclideanDistance();
...
/*	for (i = 0; i < m_rang; i++)
	...
	{
		if (m_neurons[i])
			((CNeuronKohonen*) m_neurons[i])->CalcAxonKohonen();
	}
}

ulong CLayerKohonen::GetMaxAxonIndex()
{
	ulong	i, m = 0;
	float	f, max = FLT_MIN;
	...
	for (i = 0; i < m_rang; i++)
	{
		if (m_neurons[i])
		{
			f = ((CNeuronKohonen*) m_neurons[i])->CalcAxonKohonen();
