	...
	case WM_DESTROY:
		{
			...
			{
//printf("command_dialog::WM_DESTROY delete cmnd_dlg\n");
//				delete cmnd_dlg;
			}
		}
    } 
...
command_dialog::command_dialog(win_project * p1, graphics_view * p2, const char * def_str, bool to_process)
{
	prj = p1; gv = p2;
	if (def_str) strcpy(command, def_str); else command[0] = '\0';
...
void command_dialog::OkButtonHandler(HWND hEditWnd)
{
	...
	if (prj && gv && this->to_process_command && strlen(command) > 0) prj->ProcessCommandString(gv, command);
}
