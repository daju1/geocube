...
camera::camera(const object_location & p1, fGL p2, project * p3) : dummy_object(p1)
{
	...
	fix_local_and_global_lights = false;
...
void camera::OrbitObject(const fGL * ang, const camera & cam, int type)
{
	...
	if (!cam.fix_local_and_global_lights)
	{
...
void camera::TranslateObject(const fGL * dst, const obj_loc_data * data)
{
	...
	if (!fix_local_and_global_lights)
	{
		for (i32u n1 = 0;n1 < prj->light_vector.size();n1++)
		{
...
void camera::RenderWindow(graphics_view * gv, rmode rm, int x, int y)
{
	if (gv->size[0] < 0)
	{
		//MessageBox(0, "gv->size[0] < 0","skip rendering if invalid window!!!",0);
		return;	// skip rendering if invalid window!!!
	}
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	if (rm == Draw || rm == Erase || rm == Edit || rm == Select || rm == Measure || rm == ContextMotion)
	{
...
// this "accumulation" stuff (used only in gt2 so far) is not tested after stereo mode changes...
	...
	if (rm == Draw || rm == Erase || rm == Edit || rm == EditMotion || rm == Select || rm == Measure || rm == ContextMotion)
	{
...
//}
		...
		if (rm == Edit) gv->GetProject()->EditEvent(gv, name_vector);
		if (rm == EditMotion) gv->GetProject()->EditMotionEvent(gv, name_vector);
#if 1
		// упрощаем вызов MeasureEvent()
		if (rm == Measure)
		{
			gv->GetProject()->MeasureEvent(gv, name_vector);
		}
#else
		//а это старый-оригинальный вызов MeasureEvent()
			...
			if (name_vector.size() > 1)
			{
				...
				else
				{
					switch (name_vector[0])
					{
						...
						gv->GetProject()->SelectWellElement((const WellElement *) name_vector[1]);
							break;
					} // switch
				} // test
			} //size
			else
				gv->GetProject()->SelectEvent(gv);

		}// select


		static iGLu iselected = 0;
		if (rm == ContextMotion)			
		{
			if (!(name_vector.size() > 1) || (name_vector.size() > 1 && name_vector[1] != iselected))		
			{
				gv->GetProject()->DestroyContextWindow();
				gv->useWellElementInContextMenu = false;
			}	
			...
			if (name_vector.size() > 1)			
			{
				...
				else
				{
					switch (name_vector[0])
					{
						case GLNAME_MD_TYPE_WELL_ELEMENT:
							{
								gv->GetProject()->SelectWellElement(x,y, (const WellElement *) name_vector[1], gv->GetProject()->hSurfWnd);
								gv->useWellElementInContextMenu = true;
							}
							break;
					} // switch
				} // test
				iselected = name_vector[1];
			}	
		}
	}
}
...
{
//printf("spot_light::SetupProperties\n");
...
//printf("spot_light::SetupLocation fPos = [%f %f %f %f]\n",GetLocData()->crd[0], GetLocData()->crd[1], GetLocData()->crd[2], GetLocData()->crd[3]);
}
...
{
//printf("directional_light::SetupProperties\n");
...
//printf("directional_light::SetupLocation fPos = [%f %f %f %f]\n",tmp_crd[0], tmp_crd[1], tmp_crd[2], tmp_crd[3]);
}
...
void transformer::OrbitObject(const fGL * p1, const camera & p2, int type)
{
...
{
	dummy_object::OrbitObject(p1, p2, 0);
}
