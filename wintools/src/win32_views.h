...
//	LOGFONT lf_Label;        // logical font structure
	...
	int MouseWhellNotifyHandler(HWND widget, WPARAM wParam, LPARAM lParam);	// EVENT_HANDLER
	...
	int KeyMotionNotifyHandler(HWND widget, WPARAM wParam, LPARAM lParam);	// EVENT_HANDLER
	int KeyButtonHandler(HWND widget, UINT message, WPARAM wParam, LPARAM lParam);
	...
	HCURSOR hDigitizeCursor;
	void SetDigitizeCursor(bool to_set);
	private:
...
/// A GTK-version of "##ogl_view".
#include "map_view.h"
class win_gdiplus_view : public win_view, virtual public gdiplus_view
{
	//friend class BlnProfile3D;
	//friend class BlnProfilePlane3D;
	//friend class Line3D;
	//friend class Poligon3D;
	//friend class Well_3D;
	//friend class Karotazh;

	private:
	
	static vector<win_gdiplus_view *> gdi_vector;
...
public:
	...
	//bool InitOG(HWND hWnd);
	bool InitDC(HWND hWnd);
	// my insertions ########################################
	
	win_gdiplus_view(win_project *);
	virtual ~win_gdiplus_view(void);
	...
	static win_gdiplus_view * GetGDIV(HWND);
};
...
class win_class_factory : public graphics_class_factory
{
	...
	map_view * ProduceGDIMapView(project *, Object *, bool);			// virtual
	map_view * ProduceMapView(project *, Object *, bool);			// virtual
