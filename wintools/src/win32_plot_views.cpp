...
//	printf("MakeGhemicalPlot lParam = %x\n", lParam);
	...
	wc.lpszMenuName		= MAKEINTRESOURCE(IDR_MENU_PLOT_2D_VIEW);
...
win_plot1d_view::win_plot1d_view(win_project * p1, i32s p2, const char * tit, const char * s1, const char * sv)
	: win_ogl_view(p1), plot1d_view(p1, p2, tit, s1, sv), ogl_view()
{
printf("win_plot1d_view\n");
	...
	this->view_widget = MakeGhemicalPlot(prj->GetWidget(), (void*) dynamic_cast<win_ogl_view*>(this), (char*) tit);
...
void win_plot1d_view::popup_AttachDetach(HWND widget, void *)
{
	...
	const char * tit = old_p1dv->getTitle();
	const char * s1 = old_p1dv->getN1();
	const char * sv = old_p1dv->getNV();
	bool flag = (oglv->detached == NULL);
	
	plot1d_view * new_p1dv = oglv->prj->AddPlot1DView(ud1, tit, s1, sv, flag);
...
void win_plot1d_view::CreateMyPopupMenu(HWND hWnd)
{
...
void win_plot2d_view::CreateMyPopupMenu(HWND hWnd)
{
	HMENU hMenu = CreatePopupMenu();
	//AppendMenu( hMenu, MFT_STRING, 4000, "&Prop..." );
	//AppendMenu( hMenu, MFT_SEPARATOR, 0, NULL );
...
win_rcp_view::win_rcp_view(win_project * p1, i32s p2, const char * tit, const char * s1, const char * sv)
	: win_ogl_view(p1), rcp_view(p1, p2, tit, s1, sv), ogl_view()
{
...
void win_eld_view::CreateMyPopupMenu(HWND hWnd)
{
	HMENU hMenu = CreatePopupMenu();
	//AppendMenu( hMenu, MFT_STRING, 4000, "&Prop..." );
	//AppendMenu( hMenu, MFT_SEPARATOR, 0, NULL );
