...
public:
	...
	void Init(Primitive3D& ob)
	{
		...
		for(unsigned int i = 0; i < this->m_nPoints; i++)
		{
			ptv[i] = ob_ptv[i];
			ptd[i] = ob_ptd[i];
		}
...
public:
	...
	void AllocPoints(unsigned int nPoints)
	{
...
//printf("Primitive3D::EnumObjects type = %s if ((this->*condition_fun)())\n", Object::ObjectTypeToString(this->GetObjectType()));
	...
	void Insert(unsigned int i_where, T pt)
	{
		printf("Insert(unsigned int i_where = %u, T pt [%f %f %f])", i_where, pt.x, pt.y, pt.z);
	...
	void Zoom(unsigned int ipoint, double zoomX, double zoomY, double zoomZ, double x_mean, double y_mean, double z_mean)
	{
