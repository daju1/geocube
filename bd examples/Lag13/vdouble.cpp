...
vdouble::resize(int rows, int cols) // двумерной матрицы
{
	if (m_length != rows * cols)
	{
		...
		{
			m_length = 0;
			AfxMessageBox("vdouble::resize(int, int) - Not enough memory");
			AfxThrowMemoryException();
			return;
		}
		m_szMemory +=m_length*SD;
...
vdouble::resize(int rows, int cols, const double& ra) // двумерной матрицы
{
	if (m_length != rows * cols)
	{
		...
		{
			m_length = 0;
			AfxMessageBox("vdouble::resize(int, int, double) - Not enough memory");
			AfxThrowMemoryException();
			return;
		}
		m_szMemory +=m_length*SD;
...
int vdouble::Lobachevski_Greffe_roots(vdouble& dr, Vector<complex>& cr, vdouble& unsolved,const double& epsilon_control)//Вычисляет действительные и комплексные корни действительного полинома методом Лобачевского - Греффе (для полиномов с вычленёнными кратными корнями)
{
	...
	do
	{
		...
		for (i = 0; i < len; i++)
		{
			...
			c = 0.0;
			for (j = 1; j < j2; j++)
				c += ((long double)(2*pow(-1,j))) * *(pai - j) * *(pai + j);
...
int vdouble::dwt(vdouble& t, vdouble& tsd,
				 vdouble &dh, vdouble &dg, 
				 vdouble &s, vdouble &d)
{
	if (m_length != t.m_length)
	{
...
/*		CString s;
	...
	for (i = m_length - 1; i >= len_h-1; i -= 2)// && k >= 0
	{
		*ptsd = *(pt + i - len_h + 1);
		 ...
		 vdouble& td, vdouble &d)
{
	...
	int len_h = dh.m_length,
		len_s = m_length/2 - len_h/2 + 1 + left_h2 + right_h2,
		len_d = m_length/2 - len_h/2 + 1 + left_g2 + right_g2;
	if (len_h != dg.m_length)
	{
		CString s;
		s.Format("Error using vdouble::dwt():\nlen_h(%i) != dg.m_length(%i)", 
...
/*		CString s;
	...
	int ind, i, j, it;
	for (i = m_length-1+2*right_h2; i >= len_h-1-2*left_h2; i -= 2)
	{
		it = i - len_h + 1;
		if (it < 0)
			*pts = *pt + dt*it;
		else
		{
			if (it < m_length)
				...
				*pts = *(pt - 1 + m_length) + dt*(it-m_length+1);
		}
		ind = i;// индекс сигнала, используемый в операции свёртки
		for (j = len_h - 1; j >= 0; j--)// индекс элемента вейвлет фильтра
		{
	...
	for (i = m_length-1+2*right_g2; i >= len_h-1-2*left_g2; i -= 2)// && k >= 0
	{
		it = i - len_h + 1;
		if (it < 0)
			*ptd = *pt + dt*it;
		else
		{
			if (it < m_length)
				...
				*ptd = *(pt - 1 + m_length) + dt*(it-m_length+1);
		}
		ind = i;// индекс сигнала, используемый в операции свёртки
		for (j = len_h - 1; j >= 0; j--)// индекс элемента вейвлет фильтра
		{
...
int vdouble::idwt(vdouble&  new_t, int left1_2, int right1_2, vdouble &rh_or_rg,vdouble& current_ts_or_td, vdouble &s_or_d, bool isLow)
{
	int len_hg = rh_or_rg.m_length,
		len_sd = s_or_d.m_length;

	if (len_sd != current_ts_or_td.m_length)
	{
		CString s;
		s.Format("Error using vdouble::idwt():\nlen_s(%i) != t.m_length(%i)", 
			len_sd,current_ts_or_td.m_length);
		AfxMessageBox(s);
		AfxThrowUserException();
	}
	if (len_sd < len_hg/2)
...
/*		CString s;
	...
	int len1_2 = len_sd - len_hg/2 + 1,
		ind, j;

	this->resize(2 * (len1_2+left1_2+right1_2), 0.0);
	new_t.resize(2 * (len1_2+left1_2+right1_2));

	double *psd, *phg, *pt, *pnewt, *p, sk_or_dk;
	double dt = current_ts_or_td.DiffVector().Mean() / 2.0;

	phg = rh_or_rg.pD; 
	psd = s_or_d.pD;
	pt = current_ts_or_td.pD;

#if 1
	pnewt = new_t.pD + 2*right1_2;
	p = pD + 2*right1_2;
	...
	for (int k = -right1_2; k < len1_2+left1_2; k++)
	{
		...
		else
		{
			if (it < len_sd)
				...
				*(pnewt+2*k) = *(pt - 1 + len_sd) + 2*dt*(it-len_sd+1);
		}
		*(pnewt+2*k+1) = *(pnewt+2*k) + dt;
		ind = k;
		for ( j = len_hg - 2; j>= 0; j-=2)
		{
			...
			*(p+2*k) += 2*( *(phg+j) * sk_or_dk);
			*(p+2*k+1) += 2*(*(phg+j+1) * sk_or_dk);
			ind ++;
		}
...
int vdouble::idwt(vdouble& t, vdouble&  tau, vdouble &rh, vdouble &rg, vdouble &s, vdouble &d)
{
...
/*		CString s;
	...
	int len1_2 = len_s - len_h/2 + 1,
		ind, j;

	this->resize(2 * (len1_2), 0.0);
	tau.resize(2 * (len1_2));

	double *ps,	*pd, *ph, *pg, *pt, *ptau, *p;
	double dt = t.DiffVector().Mean() / 2.0;
	...
	{
		*(ptau + 2 * k) = *(pt + k + (len_h-2)/2);
		*(ptau + 2 * k + 1) = *(ptau + 2 * k) + dt;
		ind = 0;
		...
		case 2:
			{
				...
				d_h[0] = 1.0 / 4.0 / (2.0) * (1.0 + sqrt(3.));
				d_h[1] = 1.0 / 4.0 / (2.0) * (3.0 + sqrt(3.));
				d_h[2] = 1.0 / 4.0 / (2.0) * (3.0 - sqrt(3.));
				d_h[3] = 1.0 / 4.0 / (2.0) * (1.0 - sqrt(3.));
				n00=1; m00=0.70;
			}
			break;
		case 3:
...
int vdouble::WaveletDecomposition(vdouble& t,
	  int wf, // флаг вейвлета
	  int order, // порядок вейвлета
	  int& maxLevel, // максимальный уровенть разложения
	  Vector<vdouble>& vtau,
...
int left = len_h/2+2;
	...
	while (m_length/(int)pow(2.0, (double)(maxLevel+2)) > left)
		maxLevel++;
*/
	vtau.resize(maxLevel);
	vd.resize(maxLevel);
	vs.resize(maxLevel);
	vSlow.resize(maxLevel);
	vFast.resize(maxLevel);
	vtauSF.resize(maxLevel);
...
//		d1;// обнулённые с хвостов d- коэффициентов, необходимые для восстановления низкочастотной составляющей сигнала, отделённой на каждом уровне декомпозиции
	...
	for(int j = 0; j < maxLevel; j++)
	{
		if(!current.dwt(current_t, vtau[j], dh,dg,vs[j],vd[j]))
			return 0;
		current = vs[j];
//		WriteRaporto(current.disp_str());
//		WriteRaporto(vtau[j].disp_str());
		current_t = vtau[j];
	}
...
//			AfxMessageBox(sLen);
			...
			vSlow[J].idwt(current_t, new_t, rh, rg, current_s, v0);
			current_s = vSlow[J];

			if ( j == J)
			{
				if(!vFast[J].idwt(current_t, new_t, rh, rg,v0, current_d))
					return 0;
				current_d = vFast[J];
			}
			else
			{
				if(!vFast[J].idwt(current_t, new_t, rh, rg, current_d,v0))
					return 0;
				current_d = vFast[J];
			}
			current_t = new_t;
...
/*
	...
	int left_h  = v_left_h.Size()  > 0 ? v_left_h.End()  : 0;
	int left_g  = v_left_g.Size()  > 0 ? v_left_g.End()  : 0;
	int right_h = v_right_h.Size() > 0 ? v_right_h.End() : 0;
	int right_g = v_right_g.Size() > 0 ? v_right_g.End() : 0;

	CString st;
	st.Format(
		"left_h = %d, left_g = %d, right_h = %d right_g = %d",
		left_h, left_g, right_h, right_g);
	AfxMessageBox(st);
	...
	CString str = st +
		...
		"\n\ndh2 = " + dh2.disp_str() +
		"\n\ndg2 = " + dg2.disp_str() +
		"\n\ncsdh2 = " + csdh2.disp_str() +
		"\n\ncsdg2 = " + csdg2.disp_str() +
		"\n\ncsfdh2 = " + csfdh2.disp_str() +
		"\n\ncsfdg2 = " + csfdg2.disp_str();

	AfxMessageBox(str);



	int left_h2 = left_h/2;
	int right_h2 = right_h/2;
	int left_g2 = left_g/2;
	int right_g2 = right_g/2;

	int left_f2 = left_h/2 < left_g/2 ? left_h/2 : left_g/2;
	int right_f2 = right_h/2 < right_g/2 ? right_h/2 : right_g/2;
*/
	vts.resize(maxLevel);
	vtd.resize(maxLevel);
	vd.resize(maxLevel);
	...
	vSlow.resize(maxLevel);
	vFast.resize(maxLevel);
	vtSlow.resize(maxLevel);
	vtFast.resize(maxLevel);
	...
	for(int j = 0; j < maxLevel; j++)
	{
		if(!current.dwt(current_t, left_h/2, right_h/2, dh, left_g/2, right_g/2, dg, vts[j], vs[j], vtd[j], vd[j]))
			return 0;
//		current.dwt(current_t, left_f2, right_f2, dh, left_f2, right_f2, dg, vts[j], vs[j], vtd[j], vd[j]);
		current = vs[j];
		current_t = vts[j];
	}
...
//			AfxMessageBox(sLen);
			...
			vSlow[J].idwt(vtSlow[J], left_h/2, right_h/2, rh , current_ts, current_s, true);
			current_s = vSlow[J];
			current_ts = vtSlow[J];

			if ( j == J)
			{
				if(!vFast[J].idwt(vtFast[J], left_g/2, right_g/2, rg, current_td, current_d, false))
					return 0;
				current_d = vFast[J];
				current_td = vtFast[J];
			}
			...
			{
				if(!vFast[J].idwt(vtFast[J], left_h/2, right_h/2, rh, current_td, current_d, true))
					return 0;
				current_d = vFast[J];
				current_td = vtFast[J];
			}
			 ...
			 vdouble& XF,vdouble& FI,vdouble& XP,vdouble& PSI)
{
	...
	else
	{
		for (int j=2; j < d; j++)
		{
			int pow_two = (int)pow(2,(j-1));
			P.resize(pow_two*(lenh - 1) + 1, 0.0);
			for (int i = 0; i < lenh;i++)
				P[pow_two*i] = fh[i];
			FI=2*conv(P,FI);
		}
		int pow_two = (int)pow(2,(d-1));
		...
		XP=dsequence(-(nf1+ng1)*pow(2,(d-1))+nf1, 1.0, (nf2+ng2)*pow(2,(d-1))-nf2);
		XF=dsequence(-nf1*pow(2,d)+nf1, 1.0, nf2*pow(2,d)-nf2);
